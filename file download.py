...
class download:
    ...
    x = final.check('E:\learning\itp1.1\c_test\c_test/')
    # x = ({'test.txt': '0.25', 'test_02.txt': '0.3007518796992481'}, {'test.txt': [((325, 12), (377, 10)), ((325, 12), (377, 10))], 'test_02.txt': [((22, 0), (75, 10)), ((350, 0), (403, 10))]})
    x1k = list(dictGet_key(x[0]))
    x1v = list(dictGet_value(x[0]))
    x2k = list(dictGet_key(x[1]))
    x2v = list(dictGet_value(x[1]))
    for i in range(0,len(x1k)):
        a = text_create("Document",i)
        m = str(x1k[i])
        b = data_matrix('E:\learning\itp1.1\c_test\c_test',m)
        c0 = x[0][x1k[i]]
        c1 = x[1][x1k[i]]
        a1 = mChange(c1)
        text_write(a,b,a1,c0,m)
